...
contains
    subroutine test_grid_blocks()
        ...
        expected_blocks(2,3,:,:) = reshape([ &
            0, 6, 4, &
            5, 1, 9, &
            7, 8, 3  &
        ], [3,3])  ! BM

        expected_blocks(3,3,:,:) = reshape([ &
            5, 1, 0, &
            7, 8, 3, &
            0, 6, 4  &
        ], [3,3])  ! BR
        ...
        ! Middle row
        expected_blocks(1,2,:,:) = reshape([ &
            2, 9, 6, &
            7, 0, 8, &
            5, 3, 1  &
        ], [3,3])  ! ML

        expected_blocks(2,2,:,:) = reshape([ &
            3, 5, 1, &
            9, 2, 0, &
            4, 0, 8  &
        ], [3,3])  ! MM

        expected_blocks(3,2,:,:) = reshape([ &
            4, 7, 0, &
            3, 5, 1, &
            9, 2, 6  &
        ], [3,3])  ! MR
        ...
        ! Top row
        expected_blocks(1,1,:,:) = reshape([ &
            4, 6, 7, &
            9, 1, 2, &
            0, 8, 5  &
        ], [3,3])  ! TL

        expected_blocks(2,1,:,:) = reshape([ &
            1, 0, 0, &
            8, 3, 5, &
            6, 4, 7  &
        ], [3,3])  ! TM

        expected_blocks(3,1,:,:) = reshape([ &
            8, 0, 5, &
            6, 0, 7, &
            1, 9, 2  &
        ], [3,3])  ! TR
